...
export default function ReportesPage() {
  ...
  const { fetchAreas, areas } = useAreas()




  const showBudgetReport = user?.rol?.nombre === "admin" || user?.rol?.nombre === "Auditoría"
  const showRatingsReports = user?.rol?.nombre === "admin" || user?.rol?.nombre === "responsableArea"
  const showExecutionReport = user?.rol?.nombre === "admin"
  const showActivosReport = user?.rol?.nombre === "admin" || user?.rol?.nombre === "Auditoría"
  const isConsultor = user?.rol?.nombre === "consultor"
            ...
            <Card>
              ...
              <CardContent>
                <Tabs defaultValue={showBudgetReport ? "presupuestal" : "compras"} className="w-full">
                  <TabsList
                    className={`grid w-full ${isConsultor
                      ? "grid-cols-2"
                  ...
                  {showBudgetReport && (
                    <TabsContent value="presupuestal">
                      <ReportePresupuestal
                        ...
                        fetchDatosGenerales={fetchDatosGenerales}
                        fetchPresupuestos={fetchReportePresupuestos}
                  ...
                  <TabsContent value="compras">
                    <ReporteCompras
                      ...
                      fetchHistorialCompras={fetchReporteCompras}
                      reporteComprasTotal={reporteComprasTotal}
                    />
                  </TabsContent>
                  ...
                  {(showRatingsReports || isConsultor) && (
                    <TabsContent value="proveedores">
                      <ReporteProveedores
                        fetchReporteCalificacionesProveedor={fetchReporteCalificacionesProveedor}
                  ...
                  {showRatingsReports && (
                    <TabsContent value="tesoreria">
                      <ReporteTesoreria
                        fetchReporteTesoreria={fetchReporteTesoreria}
                        reporteTesoreria={reporteTesoreria}
                      />
                    </TabsContent>
                  )}
                </Tabs>
