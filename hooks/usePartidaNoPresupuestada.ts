...
export default function usePartidaNoPresupuestada() {
  ...
  const createSolicitudPartidaNoPresupuestada = useCallback(async (registerData: RegisterRequisicionSchema) => {
    setLoadingRequisicion(true);
    setErrorRequisicion(null);

    try {
      const response = await PartidaNoPresupuestadaService.createPartidaNoPresupuestada(registerData);
      if (response.status === 201) {
        toast.success("Partida no presupuestada creada exitosamente");
        return true;
      } else {
        const errorMsg = response.message || "No se pudo crear la partida no presupuestada correctamente.";
  ...
  const fetchHistorialRequisicionesArea = useCallback(async (periodo: number, areaId: number, partidaNoPresupuestada?: boolean) => {
    setLoadingRequisicion(true);
    setErrorRequisicion(null);
