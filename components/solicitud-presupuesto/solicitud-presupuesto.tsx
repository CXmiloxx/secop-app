...
import { editSolicitudPresupuestoSchema, EditSolicitudPresupuestoSchema, RegisterSolicitudPresupuestoSchema, registerSolicitudPresupuestoSchema } from "@/schema/solicitar-presupuesto.schema"
...
interface SolicitudPresupuestoProps {
  user: UserType | null
  createSolicitud?: (solicitud: RegisterSolicitudPresupuestoSchema) => Promise<boolean>
  updateSolicitud?: (solicitud: EditSolicitudPresupuestoSchema) => Promise<boolean>
  type?: "create" | "edit"
  defaultValues?: AprobarSolicitudPresupuesto
}

export default function SolicitudPresupuesto({ user, createSolicitud, updateSolicitud, type = "create", defaultValues }: SolicitudPresupuestoProps) {
  ...
  const { conceptos, fetchConceptos, errorConceptos } = useConceptos();
  ...
  } = useForm<EditSolicitudPresupuestoSchema | RegisterSolicitudPresupuestoSchema>({
    resolver: zodResolver(type === "create" ? registerSolicitudPresupuestoSchema : editSolicitudPresupuestoSchema),
  ...
  const onSubmit = async (data: RegisterSolicitudPresupuestoSchema | EditSolicitudPresupuestoSchema) => {
    ...
    try {
      ...
      if (type === "create") {
        await createSolicitud?.(solicitudData as RegisterSolicitudPresupuestoSchema);
      } else {
        await updateSolicitud?.(solicitudData as EditSolicitudPresupuestoSchema);
      }
